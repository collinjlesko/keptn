...
then
export CLUSTERVERSION=$(curl -s https://$DT_TENANT_ID/api/v1/config/clusterversion?api-token=$DT_API_TOKEN | jq -r .version)
curl -X POST \
  "https://$DT_TENANT_ID/api/config/v1/virtualization/kubernetesConfigurations?api-token=$DT_API_TOKEN" \
