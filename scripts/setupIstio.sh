...
kubectl create -f ../../k8s-deploy-production/istio/gateway.yml
kubectl create -f ../../k8s-deploy-production/istio/destination_rule.yml
kubectl create -f ../../k8s-deploy-production/istio/virtual_service.yml

# ./createServiceEntry.sh $DT_TENANT_ID $DT_PAAS_TOKEN
if [[ $DT_TENANT_ID == *"live.dynatrace"*  ]]; then
